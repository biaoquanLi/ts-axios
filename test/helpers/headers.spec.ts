import { parseHeaders, processHeaders, falttenHeaders } from '../../src/helpers/headers'
...
describe('helpers:header', () => {
    ...
    describe('falttenHeaders', () => {
        test('should flatten the headers and include common headers', () => {
            ...
            expect(falttenHeaders(headers, 'get')).toEqual({
                Accept: 'application/json',
                'X-COMMON-HEADER': 'commonHeaderValue',
                'X-GET-HEADER': 'getHeaderValue'
            })
        ...
        test('should flatten the headers without common headers', () => {
            ...
            expect(falttenHeaders(headers, 'patch')).toEqual({
                Accept: 'application/json'
            })
        })

        test('should do nothing if headers is undefined or null', () => {
            expect(falttenHeaders(undefined, 'get')).toBeUndefined()
            expect(falttenHeaders(null, 'post')).toBeNull()
        })
    })
})